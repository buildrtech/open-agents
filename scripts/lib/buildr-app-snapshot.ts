...
export function buildBuildrAppSnapshotCommands(): string[] {
  return [
    ...
    `cd ${BUILDR_APP_LIBVIPS_SOURCE_DIRECTORY} && meson setup build --prefix=/usr/local --libdir=lib -Dintrospection=disabled && meson compile -C build && sudo meson install -C build && sudo sh -c "printf '%s\\n' /usr/local/lib > /etc/ld.so.conf.d/local-libvips.conf" && sudo ldconfig`,
    "sudo ln -sf /usr/local/bin/vips /usr/bin/vips && sudo ln -sf /usr/local/bin/vipsthumbnail /usr/bin/vipsthumbnail && sudo ln -sf /usr/local/bin/vipsheader /usr/bin/vipsheader",
    ...
    "sudo ln -sf /usr/bin/valkey-cli /usr/bin/redis-cli",
    ...
    `sudo install -m 0755 ${BUILDR_APP_AST_GREP_EXTRACTED_PATH} /usr/bin/ast-grep`,
    ...
    `sudo install -m 0755 ${BUILDR_APP_BDEV_BINARY_BUILD_PATH} /usr/bin/bdev`,
    ...
    `sudo install -m 0755 ${BUILDR_APP_MEILISEARCH_DOWNLOAD_PATH} /usr/bin/meilisearch`,
